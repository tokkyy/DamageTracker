...
FloatingTextFloat._init_node = function(self, node, t, is_dot, damage, hit_type, style_data, world_pos_box, base_size,
										duration, v_cat)
	node.active = true
	node.timer = 0
...
FloatingTextFloat._update_node_position = function(self, node, camera, s, dt, inv_scale, logical_w, logical_h)
	...
	if offset_key == "head" then
		cat_offset = s.float_head_offset
	elseif offset_key == "dot" then
		cat_offset = s.float_dot_offset
	else
		cat_offset = s.float_body_offset
	end
